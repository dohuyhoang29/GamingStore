...
public interface AccountRepositories extends CrudRepository<Account, Integer> {
  ...
  Optional<Account> findAccountByEmailAndUsername(@Param("email") String email,
      @Param("password") String password);

  @Modifying
  @Query("UPDATE Account a SET a.status = 'Lock' WHERE a.accountID = :id")
  void lockAccountById(@Param("id") Integer id);

  @Modifying
  @Query("UPDATE Account a SET a.status = 'Un-Lock' WHERE a.accountID = :id")
